...
class discrimnator(nn.Module):
    def __init__(self, df_dim=64):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(df_dim*4, df_dim*8, 4, 1, 1),
            nn.InstanceNorm2d(df_dim*8),
            nn.LeakyReLU(0.2, inplace=True),
            # output_size: n * 1 * 32 * 32
            nn.Conv2d(df_dim*8, 1, 4, 1, 1)
        )
...
class metric_net(nn.Module):
    def __init__(self, df_dim=64):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(3, df_dim, 4, 2, 1), # output_size : n * 64 * 128 * 128
            nn.LeakyReLU(0.2),
            nn.MaxPool2d(2, 2), # output_size : n * 64 * 64 * 64
            nn.Conv2d(df_dim, df_dim*2, 4, 2, 1), # output_size: n * 128 * 32 * 32
            nn.LeakyReLU(0.2),
            nn.MaxPool2d(2, 2), # output_size: n * 128 * 16 * 16
            nn.Conv2d(df_dim*2, df_dim*4, 4, 2, 1), # output_size : n * 256 * 8 * 8
            nn.LeakyReLU(0.2),
            nn.MaxPool2d(2, 2), # output_size : n * 256 * 4 * 4
            nn.Conv2d(df_dim*4, df_dim*8, 4, 2, 1), # output_size : n * 512 * 2 * 2
            nn.LeakyReLU(0.2),
...
class generator(nn.Module):
    def __init__(self, gf_dim=64):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(3, gf_dim, 7, 1), # output_size : n * 64 * 256 * 256
            nn.InstanceNorm2d(gf_dim),
            nn.ReLU(inplace=True),
            nn.Conv2d(gf_dim, gf_dim*2, 3, 2, 1), # output_size : n * 128 * 128 * 128
            nn.InstanceNorm2d(gf_dim*2),
            nn.ReLU(inplace=True),
            nn.Conv2d(gf_dim*2, gf_dim*4, 3, 2, 1), # output_size : n * 256 * 64 * 64
